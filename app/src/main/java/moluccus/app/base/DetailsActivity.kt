...
import moluccus.app.databinding.DetailLayoutBinding
...
class DetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.toolbar.title = "Complete Profile"
        initLayout()
        intialLocationTest()
    }
    ...
    private fun uploadInitDataUpdate(
        ...
        firebaseDatabase.child("users").child(uid).child("usr_information")
    ...
    private fun onImageSelected(eri: Uri) {
        binding.uploadCover.visibility = View.VISIBLE
        val storageReference = FirebaseUtils.firebaseStorage
            .child("galleries")
            .child(uid)
            .child("$uid-cover-photo")
    ...
    private fun getPhoto() {
        firebaseDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val photo = snapshot.child("users").child(FirebaseUtils.firebaseUser!!.uid)
                ...
                val pp = snapshot.child("users").child(FirebaseUtils.firebaseUser.uid)
